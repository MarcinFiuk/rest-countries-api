...
function DisplayIndividualCountry() {
  ...
  const { data, error, isLoading } = useAxios(
    ...
    {
      fields:
        "flags,name,nativeName,population,region,subregion,capital,topLevelDomain,currencies,languages,borders",
    }
  );

  const countriesStore = useContext(countriesContext);

  if (isLoading || countriesStore.isLoading) {
    return <Spinner />;
  }

  if (error || countriesStore.error) {
    return <Error errorInfo={countriesStore.error} />;
  }

  // [ { br: "Brasil" }, { ar: "Argentina" } ]
  // { br: "Brasil", ar: "Argentina" }

  const renderBorders = (borders = []) => {
    if (borders.length > 0) {
      return borders.map((border, index) => {
        const fullName = countriesStore.data.countriesName[border];
        return (
              ...
              <CountryWrapper>
                ...
                <TextWrapper>
                  ...
                  <InfoColumns>
                    <FirstInfoColumn>
                      <p>
                        <BoldWrapper>Native Name:&nbsp;</BoldWrapper>
                        {nativeName}
                      </p>
                      <p>
                      ...
                      <p>
                        <BoldWrapper>Region:&nbsp;</BoldWrapper>
                        {region}
                      </p>
                      <p>
                        <BoldWrapper>Sub Region:&nbsp;</BoldWrapper>
                        {subregion}
                      </p>
                      <p>
                        <BoldWrapper>Capital:&nbsp;</BoldWrapper>
                        {capital}
                      </p>
                    </FirstInfoColumn>
                    <SecondInfoColumn>
                      <p>
                        <BoldWrapper>Top Level Domain:&nbsp;</BoldWrapper>
                        {topLevelDomain[0]}
                      </p>
                      <p>
                        <BoldWrapper>Currencies:&nbsp;</BoldWrapper>
